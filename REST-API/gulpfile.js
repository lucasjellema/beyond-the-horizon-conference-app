...
var appname = 'Data-API';
var apparchive = "REST-API.zip";

//gulp.task('dist', gulp.series(_clean, _rjs, _zip));
gulp.task('dist', gulp.series(_clean, _zip));
...
gulp.task('redeploy', gulp.series( _redeploy2));
...
function _zip() {
    return gulp.src(['**/*', '!ocloud*.js', '!gulpfile.js','!node_modules/**/*', '**/express/**/*' ])
        .pipe(zip(apparchive))
        .pipe(gulp.dest('dist'));
}
...
function _deploy() {
    createApp(appname, fs.createReadStream('manifest.json'), fs.createReadStream('deployment.json'), fs.createReadStream('dist/'+apparchive),
...
function _redeploy() {
    updateApp(appname, fs.createReadStream('dist/'+apparchive), require('./ocloud'), response => { console.log('UPDATED', response) });
}

function _redeploy2() {
    updateApp(appname, fs.createReadStream(apparchive), require('./ocloud'), response => { console.log('UPDATED', response) });
}
...
function createApp(name, manifest, deployment, archive, ocloud, callback) {
    ...
    form.append('subscription', 'Monthly');
